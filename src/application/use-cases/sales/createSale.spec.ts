...
describe("Create Sale", () => {
  ...
  it("should create a new sale with success", () => {
    expect(async () => await createSale.execute(Sale.create(saleData))).not.toThrow();
  });

  it("should retrive created sale", async () => {
    await createSale.execute(Sale.create(saleData));

    const products = await getProducts.execute(restaurantData.id);

    expect(products.length).toBe(1);
    expect(products[0].sale).not.toBeNull();
  });

  it("should throw error when product not found", async () => {
    const invalidSaleData = { ...saleData, productId: "invalidProductId" };

    expect(async () => await createSale.execute(Sale.create(invalidSaleData))).rejects.toThrow();
  });
});
