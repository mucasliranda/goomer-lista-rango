...
describe("Delete Sale", () => {
  ...
  const deleteSale = new DeleteSale(databaseSaleRepository);
  ...
  it("should delete a sale with success", () => {
    expect(async () => await deleteSale.execute(saleData.id)).not.toThrow();
  });

  it("should not retrive deleted sale", async () => {
    await deleteSale.execute(saleData.id);

    const products = await getProducts.execute(restaurantData.id);

    expect(products[0].sale).toBeNull();
  });
});
